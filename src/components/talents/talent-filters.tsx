...
export function TalentFilters({ category, level, search }: TalentFiltersProps) {
  return (
    <form className="mt-6 grid gap-3 rounded-2xl border border-[#e7ebe8] bg-white p-4 shadow-sm md:grid-cols-[minmax(0,1fr)_180px_180px_auto]">
      ...
      <label className="sr-only" htmlFor="talent-level">按达人等级筛选</label>
      <select className="rounded-lg border border-[#dfe5e1] bg-white px-3 py-2 text-sm text-[#35443e]" defaultValue={level} id="talent-level" name="level" onChange={(event) => event.currentTarget.form?.requestSubmit()}>
        <option value="">全部达人等级</option>
        {TALENT_LEVELS.map((value) => <option key={value} value={value}>{TALENT_LEVEL_LABELS[value]}</option>)}
      </select>
      <div className="flex gap-2">
        ...
        {search || category || level ? <Link className="inline-flex items-center px-2 text-sm text-slate-500 hover:text-[#31594b]" href="/talents">清除</Link> : null}
      </div>
    </form>
  );
}
