...
type Props = { continueProcessing?: boolean; nextResourceId?: string; processedCount?: number; processingDone?: boolean; processingScope?: "today"; resourceId: string; totalCount?: number; workQueue?: boolean };
...
function SubmitButton({ continueProcessing, workQueue }: { continueProcessing: boolean; workQueue: boolean }) {
  const { pending } = useFormStatus();
  return <button className="rounded-lg bg-[#31594b] px-4 py-2.5 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:bg-[#91a59e]" disabled={pending} type="submit">{pending ? "正在保存…" : continueProcessing || workQueue ? "保存并处理下一项" : "保存联系记录"}</button>;
}

export function ResourceContactForm({ continueProcessing = false, nextResourceId, processedCount = 0, processingDone = false, processingScope, resourceId, totalCount = 0, workQueue = false }: Props) {
  ...
  return <form action={createResourceContactRecord} className="mt-4 grid gap-4 rounded-xl border border-[#e4e9e6] bg-[#f8faf8] p-4">
    ...
    {workQueue ? <input name="work_queue" type="hidden" value="1" /> : null}
    ...
    <div className="flex flex-col items-start justify-between gap-3 border-t border-[#dfe5e1] pt-4 sm:flex-row sm:items-center"><p className="text-xs text-slate-400">{workQueue ? "保存成功后会自动进入下一项今日工作" : continueProcessing ? "保存成功后会自动进入下一条资源" : "保存后留在当前资源详情"}</p><SubmitButton continueProcessing={continueProcessing} workQueue={workQueue} /></div>
  </form>;
}
