...
type Props = { continueProcessing?: boolean; processingScope?: "today"; resourceId: string };

const STATUS_HINTS: Partial<Record<(typeof RESOURCE_CONTACT_RESULTS)[number], string>> = {
  friend_request: "自动更新为“等待通过”，2 天后再次处理",
  reapplication: "自动更新为“等待通过”，2 天后再次处理",
  accepted: "自动更新为“已联系”，明天继续处理",
  replied: "自动更新为“已联系”，明天继续处理",
  rejected: "自动更新为“暂不处理”，不再安排时间",
  no_response: "自动更新为“已尝试添加”，2 天后再次处理",
};

const NEXT_ACTION_DAYS: Partial<Record<(typeof RESOURCE_CONTACT_RESULTS)[number], number>> = {
  friend_request: 2,
  reapplication: 2,
  accepted: 1,
  replied: 1,
...
function getSuggestedNextAction(value: (typeof RESOURCE_CONTACT_RESULTS)[number]) {
  const days = NEXT_ACTION_DAYS[value];
  return days === undefined ? "" : toShanghaiDateTimeLocalValue(new Date(Date.now() + days * 24 * 60 * 60 * 1000));
}
...
export function ResourceContactForm({ continueProcessing = false, processingScope, resourceId }: Props) {
  const [result, setResult] = useState<(typeof RESOURCE_CONTACT_RESULTS)[number]>("friend_request");
  const [nextActionAt, setNextActionAt] = useState(getSuggestedNextAction("friend_request"));

  function chooseResult(value: (typeof RESOURCE_CONTACT_RESULTS)[number]) {
    setResult(value);
    setNextActionAt(getSuggestedNextAction(value));
  }
