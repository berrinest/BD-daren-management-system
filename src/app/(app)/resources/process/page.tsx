...
type Props = {
  searchParams: Promise<{ after?: string; error?: string; notice?: string; resource?: string }>;
};
...
export default async function ResourceProcessPage({ searchParams }: Props) {
  ...
  const previousIndex = params.after
    ...
    : -1;
  const currentResource = requestedResourceIndex >= 0
    ? resourceList[requestedResourceIndex]
    : params.after && previousIndex >= 0
      ? resourceList[previousIndex + 1]
      : params.after
        ? undefined
        : resourceList[0];
  ...
  return <main className="p-5 md:p-8"><section className="mx-auto max-w-5xl">
    ...
    {params.notice === "contact-created" ? <p className="mt-5 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800" role="status">上一条联系记录已保存，已进入下一条。</p> : null}
    {!currentResource ? <div className="mt-6 rounded-2xl border border-[#e7ebe8] bg-white px-6 py-16 text-center shadow-sm"><h2 className="text-xl font-semibold text-[#26332e]">本轮资源已处理完成</h2><p className="mt-2 text-sm text-slate-500">可以返回工作台，或重新开始一轮检查。</p><div className="mt-6 flex justify-center gap-3"><Link className="rounded-lg bg-[#31594b] px-4 py-2.5 text-sm font-semibold text-white" href="/dashboard">返回工作台</Link><Link className="rounded-lg border border-[#d6dfda] px-4 py-2.5 text-sm font-medium text-[#31594b]" href="/resources/process">重新开始</Link></div></div> : <>
      <section className="mt-6 rounded-2xl border border-[#e7ebe8] bg-white p-6 shadow-sm md:p-8">
        <div className="flex flex-col justify-between gap-3 sm:flex-row"><div><p className="text-xs text-slate-400">当前第 {Math.max(requestedResourceIndex, previousIndex + 1, 0) + 1} / {resourceList.length} 条</p><h2 className="mt-2 text-2xl font-semibold text-[#26332e]">{currentResource.nickname}</h2><p className="mt-2 text-sm text-slate-500">{getTalentPlatformLabel(currentResource.primary_platform)} · {currentResource.category}</p></div><Link className="text-sm font-medium text-[#31594b] hover:underline" href={`/resources/${currentResource.id}`}>查看完整资源详情</Link></div>
        <dl className="mt-6 grid gap-3 border-t border-[#edf0ee] pt-5 sm:grid-cols-2 lg:grid-cols-4"><div className="rounded-xl bg-[#f8faf8] p-4"><dt className="text-xs text-slate-400">平台账号</dt><dd className="mt-1 text-sm font-medium">{currentResource.platform_account || "未填写"}</dd></div><div className="rounded-xl bg-[#f8faf8] p-4"><dt className="text-xs text-slate-400">微信号</dt><dd className="mt-1 text-sm font-medium">{currentResource.wechat || "未填写"}</dd></div><div className="rounded-xl bg-[#f8faf8] p-4"><dt className="text-xs text-slate-400">优先级</dt><dd className="mt-1 text-sm font-medium">{getTalentPriorityLabel(currentResource.priority)}</dd></div><div className="rounded-xl bg-[#f8faf8] p-4"><dt className="text-xs text-slate-400">处理状态</dt><dd className="mt-1 text-sm font-medium">{getResourceProcessingStatusLabel(currentResource.processing_status)}</dd></div></dl>
        {currentResource.profile_url ? <a className="mt-5 inline-flex text-sm font-medium text-[#31594b] hover:underline" href={currentResource.profile_url} rel="noreferrer" target="_blank">打开达人主页 ↗</a> : null}
        <p className="mt-5 whitespace-pre-wrap text-sm leading-7 text-slate-600">{currentResource.notes || "暂无资源备注"}</p>
      </section>
      <section className="mt-6 rounded-2xl border border-[#e7ebe8] bg-white p-6 shadow-sm md:p-8"><h2 className="text-lg font-semibold text-[#26332e]">记录本次联系</h2><ResourceContactForm continueProcessing resourceId={currentResource.id} /></section>
