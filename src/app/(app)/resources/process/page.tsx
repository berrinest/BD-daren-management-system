...
type Props = {
  searchParams: Promise<{ after?: string; error?: string; notice?: string; resource?: string; statusUpdated?: string }>;
};
...
export default async function ResourceProcessPage({ searchParams }: Props) {
  ...
  const processableResources = resourceList.filter((item) => item.processing_status !== "paused");
  ...
  const currentResource = requestedResource && requestedResource.processing_status !== "paused"
    ? requestedResource
    : params.after && previousIndex >= 0
      ? resourceList.slice(previousIndex + 1).find((item) => item.processing_status !== "paused")
      : params.after
        ? undefined
  ...
  return <main className="p-5 md:p-8"><section className="mx-auto max-w-5xl">
    ...
    {params.notice === "contact-created" ? <p className="mt-5 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800" role="status">上一条联系记录已保存{params.statusUpdated === "1" ? "，处理状态已自动更新" : ""}，已进入下一条。</p> : null}
