...
import { ResourceWechatForm } from "@/components/resources/resource-wechat-form";
...
type Props = {
  ...
  searchParams: Promise<{ error?: string; notice?: string; returnTo?: string; statusUpdated?: string }>;
};
...
export default async function ResourceDetailPage({ params, searchParams }: Props) {
  ...
  const returnToWork = message.returnTo === "work";
  ...
  const { data: latestContactRecord } = await supabase
  ...
  const processingTimes = [
    latestContactRecord?.occurred_at,
    new Date(resource.updated_at).getTime() > new Date(resource.created_at).getTime() ? resource.updated_at : null,
  ].filter((value): value is string => Boolean(value));
  const recentProcessingAt = processingTimes.sort((left, right) => new Date(right).getTime() - new Date(left).getTime())[0];
  ...
  return <main className="p-5 md:p-8"><section className="mx-auto max-w-4xl">
    <Link className="text-sm font-medium text-[#557064] hover:underline" href={returnToWork ? "/work" : "/resources"}>← {returnToWork ? "返回今日工作" : "返回资源池"}</Link>
    ...
    {message.notice === "wechat-updated" ? <p className="mt-5 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800" role="status">微信号已保存。</p> : null}
    ...
    <div className="mt-6 rounded-2xl border border-[#e7ebe8] bg-white p-6 shadow-sm md:p-8">
      ...
      <dl className="mt-7 grid gap-4 border-t border-[#edf0ee] pt-6 sm:grid-cols-2 lg:grid-cols-4">{details.map(({ copyValue, label, value }) => <div className="rounded-xl bg-[#f8faf8] p-4" key={label}><dt className="text-xs text-slate-400">{label}</dt><dd className="mt-1.5 flex items-center justify-between gap-2 text-sm font-medium text-[#35443e]"><span className="min-w-0 truncate">{value}</span>{copyValue ? <CopyButton value={copyValue} /> : null}</dd></div>)}<div className="rounded-xl bg-[#f8faf8] p-4 sm:col-span-2 lg:col-span-1"><dt className="text-xs text-slate-400">微信号</dt><dd className="mt-2"><ResourceWechatForm editable={resource.status === "new"} resourceId={resource.id} wechat={resource.wechat} /></dd></div></dl>
      <section className="mt-6 border-t border-[#edf0ee] pt-6"><h2 className="text-sm font-semibold text-[#35443e]">发现与处理信息</h2><dl className="mt-3 grid gap-3 sm:grid-cols-3"><div className="rounded-xl bg-[#f8faf8] p-4"><dt className="text-xs text-slate-400">来源记录</dt><dd className="mt-1.5 whitespace-pre-wrap text-sm font-medium text-[#35443e]">{resource.source || "未填写"}</dd></div><div className="rounded-xl bg-[#f8faf8] p-4"><dt className="text-xs text-slate-400">首次发现时间</dt><dd className="mt-1.5 text-sm font-medium text-[#35443e]">{formatDateTime(resource.discovered_at)}</dd></div><div className="rounded-xl bg-[#f8faf8] p-4"><dt className="text-xs text-slate-400">最近处理时间</dt><dd className="mt-1.5 text-sm font-medium text-[#35443e]">{recentProcessingAt ? formatDateTime(recentProcessingAt) : "尚未处理"}</dd></div></dl></section>
