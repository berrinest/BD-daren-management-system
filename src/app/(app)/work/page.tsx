    ...
    {!currentItem ? <section className="mt-6 rounded-2xl border border-[#e7ebe8] bg-white px-6 py-16 text-center shadow-sm"><h2 className="text-xl font-semibold text-[#26332e]">今日工作已处理完成</h2><p className="mt-2 text-sm text-slate-500">当前没有逾期、今日到期或新资源事项。</p><Link className="mt-6 inline-flex rounded-lg bg-[#31594b] px-4 py-2.5 text-sm font-semibold text-white" href="/dashboard">返回工作台</Link></section> : <>
      <section className="mt-6 rounded-2xl border border-[#e7ebe8] bg-white p-6 shadow-sm md:p-8">
        ...
        {currentItem.kind === "talent_task" ? <>
          ...
          <div className="mt-6 flex flex-wrap items-center justify-between gap-3 border-t border-[#edf0ee] pt-6"><Link className="text-sm font-medium text-[#31594b] hover:underline" href={`/talents/${currentItem.talentId}?task=${currentItem.taskId}&returnTo=work`}>打开达人完整详情 →</Link>{currentItem.talentId && currentItem.taskId ? <TaskActions returnTo="work" talentId={currentItem.talentId} taskId={currentItem.taskId} /> : null}</div>
        </> : <>
          ...
          <div className="mt-5"><Link className="text-sm font-medium text-[#31594b] hover:underline" href={`/resources/${currentItem.resourceId}?returnTo=work`}>打开资源完整详情 →</Link></div>
        </>}
      </section>
