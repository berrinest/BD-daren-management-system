...
export default async function TalentsPage({ searchParams }: TalentsPageProps) {
  const { category: rawCategory, level: rawLevel, notice, q = "" } = await searchParams;
  const search = q.trim().slice(0, 100);
  const category = TALENT_CATEGORIES.find((value) => value === rawCategory) ?? "";
  const level = TALENT_LEVELS.find((value) => value === rawLevel) ?? "";
  ...
  let query = supabase
    .from("talents")
    .select("id, nickname, primary_platform, platform_account, wechat, tags, talent_level, stage, created_at")
    .eq("user_id", userId)
    .is("archived_at", null)
      ...
      <section className="mx-auto max-w-6xl">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-500">管理达人资料、达人等级和当前合作阶段。</p>
          </div>
        ...
        <TalentFilters category={category} level={level} search={search} />
        ...
        {!error && talents?.length === 0 ? (
          <div className="mt-5 rounded-2xl border border-dashed border-[#d9e1dc] bg-white px-6 py-14 text-center">
            <h2 className="text-base font-semibold text-[#35443e]">{search || category || level ? "没有找到匹配的达人" : "还没有达人记录"}</h2>
            <p className="mt-2 text-sm text-slate-500">{search || category || level ? "调整搜索或筛选条件后再试。" : "添加第一位达人，开始管理你的 BD 跟进。"}</p>
          </div>
        ) : null}
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[900px] border-collapse text-left text-sm">
                <thead className="bg-[#f8faf8] text-xs font-semibold tracking-wide text-[#668074]">
                  <tr><th className="px-5 py-3">达人</th><th className="px-4 py-3">平台</th><th className="px-4 py-3">微信</th><th className="px-4 py-3">赛道</th><th className="px-4 py-3">达人等级</th><th className="px-4 py-3">阶段</th><th className="px-5 py-3 text-right">操作</th></tr>
                </thead>
