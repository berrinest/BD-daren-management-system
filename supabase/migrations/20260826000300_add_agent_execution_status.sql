alter table public.tasks
  add column agent_execution_status text;

update public.tasks
set agent_execution_status = 'claimed'
where status = 'in_progress';

alter table public.tasks
  add constraint tasks_agent_execution_status_valid check (
    agent_execution_status is null
    or agent_execution_status in ('claimed', 'running', 'failed')
  ),
  add constraint tasks_agent_execution_lifecycle_valid check (
    (status = 'pending' and agent_execution_status is null)
    or (status = 'in_progress' and agent_execution_status is not null)
    or status in ('completed', 'cancelled')
  );

create index tasks_user_agent_execution_status_idx
on public.tasks (user_id, agent_execution_status, updated_at desc)
where agent_execution_status is not null;

comment on column public.tasks.agent_execution_status is
  'Agent-side lifecycle state. Business completion remains represented by tasks.status.';
...
begin
  ...
  select t.talent_id, t.resource_id
  into v_talent_id, v_resource_id
...
comment on function public.complete_agent_task_result(
  ...
  timestamptz,
  timestamptz
) is 'Atomically applies an Agent result through the existing target-domain workflow and completes the claimed task.';
